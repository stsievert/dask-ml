...
def stop_on_plateau(info, history, patience=10, tol=0.001, max_iter=None):
    out = {}
...
class _SHA:
    ...
    def fit(self, info, history):
        ...
        keep_training = stop_on_plateau(info, history,
        ...
        if self.steps > self.limit or (self.limit is None and len(best) in {0, 1}):
            best_scores = {h['model_id']: {"score": -np.inf, "calls": -np.inf}
                           for h in history}
            for h in history:
                if best_scores[h['model_id']]['calls'] < h['partial_fit_calls']:
                    best_scores[h['model_id']] = {'score': h['score'], 'calls': h['partial_fit_calls']}
            best_scores = {k: v['score'] for k, v in best_scores.items()}
            max_score = max(best_scores.values())
            best_ids = {k for k, v in best_scores.items() if v == max_score}
